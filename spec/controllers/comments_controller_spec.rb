...
RSpec.describe CommentsController, type: :controller do
    describe "comments#create action" do 
        it "should allow users to create comments on grams" do 
            # gram needs to exist, thus created
            gram = FactoryBot.create(:gram)
            
            user = FactoryBot.create(:user)
            sign_in user

            #when someone who is not  logged in triggers POST request to a URL that looks like /grams/:gram_id/comments with the message "awesome gram"
        ...
        it "should return http status code of not dounf if the gram isn't found" do 
            user = FactoryBot.create(:user)
            sign_in user
